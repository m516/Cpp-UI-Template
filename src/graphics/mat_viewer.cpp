...
MatViewer::MatViewer(const std::string &name, Mat& mat) :
mat(&mat), name(name) {
...
void MatViewer::addToGUI(bool withControls, bool withTooltip)
{
	...
	if(!initialized()) {
		...
		if(mat == NULL){
			ImGui::Text("The image is null");
			return;
		} 
		if(mat->empty()) ImGui::Text("The image is empty");
		return;
	}
	...
	//Display scale
	ImGui::SliderFloat((name + " Display Scale").c_str(), 
		&imageScale, 
		0.1, 5, 
		"%.3f", 
